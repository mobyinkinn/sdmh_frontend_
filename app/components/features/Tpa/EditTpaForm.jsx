...
function EditTpaForm({ onCloseModal, id }) {
  ...
  const [name, setName] = useState(filteredData[0].name);
  const [tag, setTag] = useState(filteredData[0].tag || "");
  ...
  function onUpdateDepartment(updatedData, id) {
    updateTpa({ id, data: updatedData });
  }
      ...
      <FormRow label="Name" error={errors?.name?.message}>
        <Input
          ...
          onChange={(e) => {
            const newName = e.target.value;
            setName(newName);
            onUpdateDepartment({ name: newName, tag }, id);
          }}
        />
      </FormRow>
      ...
      <FormRow label="Tag" error={errors?.tag?.message}>
        <Input
          ...
          onChange={(e) => {
            ...
            setTag(newTag);
            onUpdateDepartment({ name, tag: newTag }, id);
          }}
        />
      </FormRow>
